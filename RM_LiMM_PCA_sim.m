...
%% Permutation test Pepe
% if options.permute == 'yes'
% 
%     teststat_perm_obs_SSQ = sum(sum((varargout{2}.M).^2));
%     teststat_perm_obs_T_SSQ = sum(sum((varargout{2}.scores).^2));
%     teststat_perm_obs_F_ratio = (sum(sum((varargout{2}.M).^2)))/(sum(sum((varargout{5}).^2))/(100-rank(X)));
% 
%     vp = repmat(1:50,2,1);
%     vp = vp(:);
%     up = unique(vp);
...
%         teststat_perm_SSQ(i) = sum(sum((M_treatment_perm).^2));
%         teststat_perm_T_SSQ(i) = sum(sum((scores_perm).^2));
%         teststat_perm_F_ratio(i) = sum(sum((M_treatment_perm).^2))/(sum(sum((E_perm).^2))/(100-rank(X)));
%     end
% 
%     varargout{2}.pval_perm4 = (sum(teststat_perm_SSQ > teststat_perm_obs_SSQ))/options.iterations;
%     varargout{2}.pval_perm5 = (sum(teststat_perm_T_SSQ > teststat_perm_obs_T_SSQ))/options.iterations;
%     varargout{2}.pval_perm6 = (sum(teststat_perm_F_ratio > teststat_perm_obs_F_ratio))/options.iterations;
% end
